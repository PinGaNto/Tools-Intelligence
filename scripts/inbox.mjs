...
//   1. Fetches the source link the submitter gave, the same way scripts/scan.mjs
//      fetches official pages.
//   2. Sends the fetched page text + the submitter's own note to Groq, and asks
//      it to either produce a properly formatted Updates-page entry grounded in
//      the actual source content, or reject the submission (spam, unrelated,
//      unsubstantiated, etc.)
...
function loadData() {
  ...
  vm.runInContext(
    src + '\nthis.__out = { TOOLS, UPDATES, TRENDING, ISSUES, TRENDING_REFRESHED_AT, ISSUES_REFRESHED_AT, SCAN_META: (typeof SCAN_META!=="undefined"?SCAN_META:{lastRun:null,runType:null}), INBOX_PROCESSED: (typeof INBOX_PROCESSED!=="undefined"?INBOX_PROCESSED:[]) };',
    sandbox
  );
...
async function main() {
  ...
  for (const row of dataRows) {
    ...
    if (!sourceLink || !/^https?:\/\//i.test(sourceLink)) {
      console.log(`  ✗ [${timestamp}] no valid source link — rejected automatically`);
      rejected++;
      continue;
    }

    console.log(` - [${timestamp}] "${tool || '(no tool given)'}" from "${name || 'anonymous'}" (${sourceLink})`);
    let result;
    try {
    ...
    const item = {
      ...
      reliabilityNote: `Submitted via the tools inbox${firstName ? ` by ${firstName} (team)` : ''}. Verify against the source link before treating as confirmed.`,
      submitted: { isTeam: !!firstName, firstName: firstName || null },
    };
    data.UPDATES.push(item);
    accepted++;
    console.log(`  ✓ accepted as Update #${item.id}${firstName ? ` (team: ${firstName})` : ' (community)'}`);
    await sleep(2000);
  }
